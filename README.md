...
- **布局**：`--layout {auto,stack,1+1,1+2,2+2}`（或 `LAYOUT` 环境变量）。auto 按当前请求成功的 provider 数自适应（2→1+1、3→1+2、≥4→2+2；鉴权失败/超时的 provider 会被隐藏并缩小布局）；1+2 = 上半格 + 下部两格；2+2 = 四格，刷新时间显示在屏幕右上角
